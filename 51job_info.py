...
def get_html(url):
    ...
    chrome_driver = Service(executable_path=r'./chromedriver.exe')
    ...
    slider = driver.find_element(By.XPATH,'//div[@class="nc_bg"]')
...
def get_msg(excel1, sheet1):
    ...
    #连接数据库 建立游标
    conn = pymysql.connect(host='10.101.0.60', user='nvidia_test', password='nvidia-test', db='tmp')
    cursor = conn.cursor()

    for i in range(1, 2):  # 页数自己随便改
        # try:
        ...
        for dbitem in dbitems:
            ...
            dbitem=list(dbitem)
            # dbitem.insert(0,num)
            # insert2database(dbitem)
            # 执行一个SQL语句
            sql = "INSERT INTO `51job` (`position`,`companyName`,`companyLocation`,`companyCharacter`,`salary`,`educationBackground`,`workExperience`,`companyScale`,`releaseTime`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            # sql = "INSERT INTO `51job` VALUES (%s,%s)"
            cursor.execute(sql, dbitem)
            conn.commit()
        ...
        # 执行一个SQL语句
    #     sql = "INSERT INTO `51job` (`no`,`position`,`companyName`,`companyLocation`,`companyCharacter`,`salary`,`educationBackground`,`workExperience`,`companyScale`,`releaseTime`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    #     cursor.execute(sql, no, position, companyName, companyLocation, companyCharacter, salary, educationBackground,
    #                    workExperience, companyScale, releaseTime)
    #     conn.commit()
        # 关闭连接
    conn.close()
